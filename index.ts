...
//Client
/*
Lock - это умный замок, который получает id ключ карты работника в функию getAccess,
которая обращает к базе данных через proxy(Паттерн заместителя) за получением должности работника.
Lock открывается, когда работник имеет должность CEO, и закрывается, когда должность другая(ниже).

Proxy имеет кэш за последние открытия двери, к которому прежде обращается дверь прежде чем обращаться к самой двери.
*/
class Lock{
    open : boolean = false;
